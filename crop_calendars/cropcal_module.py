...
def adjust_grainC(da_in, patches1d_itype_veg_str):
    ...
    drymatter_fractions = {
        ...
        "sugarcane": (
            1 - 0.745
        ),  # Irvine, Cane Sugar Handbook, 10th ed., 1977, P. 16. (see sugarcane.py)
        "wheat": 0.88,
    }
    ...
    if not np.array_equal(np.isnan(wet_tp), np.isnan(da_in.values)):
        missing_croptypes = [
            x for x in np.unique(patches1d_itype_veg_str) if x not in drymatter_cropList
        ]
        raise RuntimeError(
            f"Failed to completely fill wet_tp. Missing crop types: {missing_croptypes}"
        )

    # Save to output DataArray
    da_out = xr.DataArray(data=wet_tp * fyield / cgrain, coords=da_in.coords, attrs=da_in.attrs)
    return da_out
...
def adjust_gridded_lonlats(
    patches1d_lonlat, patches1d_ij, lu_dsg_lonlat_da, this_tolerance, i_or_j_str
):
    ...
    lu_dsg_lonlat_da = xr.DataArray(data=new_gridded_lonlats, coords={"lat": new_gridded_lonlats})
...
def check_and_trim_years(y1, yN, ds_in):
    ...
    if get_year_from_cftime(ds_in.time.values[0]) > y1:
        raise RuntimeError(
            f"Requested y1 is {y1} but first year in outputs is"
            f" {get_year_from_cftime(ds_in.time.values[0])}"
        )
    elif get_year_from_cftime(ds_in.time.values[-1]) < y1:
        raise RuntimeError(
            f"Requested yN is {yN} but last year in outputs is"
...
def check_case_earthstats_dim_match(case, earthstats_ds, dim):
    ...
    dim_used_in_earthstats = dim in earthstats_ds and np.any(
        [dim in earthstats_ds[x].dims for x in earthstats_ds]
    )
    if dim_used_in_case:
...
def check_constant_vars(
    ...
    if constantGSs:
        ...
        if constantGSs.start > gs0 or constantGSs.stop < gsN:
            print(
                f"❗ Only checking constantVars over {constantGSs.start}-{constantGSs.stop} (run"
                f" includes {gs0}-{gsN})"
            )
                ...
                if not np.all(ok_p):
                    ...
                    for c in np.unique(varyCrops_int):
                        ...
                        if len(theseRxVals) != len(varyLats_thisCrop):
                            raise RuntimeError(
                                f"Expected {len(varyLats_thisCrop)} rx values; got"
                                f" {len(theseRxVals)}"
                            )
                    ...
                    for i, p in enumerate(bad_patches_thisT):
                        ...
                        if verbose:
                            thisStr = (
                                f"   Patch {thisPatch} (lon {thisLon} lat {thisLat})"
                                f" {thisCrop} ({thisCrop_int})"
                            )
                            ...
                            if v == "SDATES":
                                strList.append(
                                    f"{thisStr}: Sowing {t1_yr} jday {t1_val_print}, {t_yr} jday"
                                    f" {t_val_print}"
                                )
                            else:
                                strList.append(
                                    f"{thisStr}: {t1_yr} {v} {t1_val_print},"
                                    f" {t_yr} {v} {t_val_print}"
                                )
                        else:
                            if ok:
    ...
    for vegtype_str in vegtype_list:
        ...
        if np.any(sim_array != rx_array):
            ...
            if output_var == "GDDHARV_PERHARV":
                ...
                diff_array = np.ma.masked_array(
                    diff_array, mask=(diff_array < 0) & (ds_thisVeg["HARVEST_REASON"].values == 5)
                )
            ...
            if np.any(np.abs(diff_array[abs(diff_array) > 0]) > 0):
                ...
                diffs_eg_txt = (
                    f"{vegtype_str} ({vegtype_int}): diffs range {min_diff} (lon {minLon}, lat"
                    f" {minLat}, gs {minGS}, rx ~{minRx}) to {max_diff} (lon {maxLon}, lat"
                    f" {maxLat}, gs {maxGS}, rx ~{maxRx})"
                )
                if "GDDHARV" in output_var:
                    diffs_eg_txt += (
...
def check_v0_le_v1(this_ds, vars, msg_txt=" ", both_nan_ok=False, throw_error=False):
    ...
    if np.all(gdd_lt_hui):
        ...
        msg = (
            msg
            + f"\ne.g., patch {p}: {this_ds.patches1d_itype_veg_str.values[p]}, lon"
            f" {this_ds.patches1d_lon.values[p]} lat {this_ds.patches1d_lat.values[p]}:"
        )
        msg = msg + f"\n{this_ds[v0].values[p,:]}"
        msg = msg + f"\n{this_ds[v1].values[p,:]}"
        if throw_error:
...
def convert_axis_time2gs(this_ds, verbose=False, myVars=None, incl_orig=False):
    ...
    def pym_to_pg(pym, quiet=False):
        ...
        if not quiet:
            print(
                ...
                f" {np.unique(np.sum(~np.isnan(pg), axis=1))}"
            )
        return pg
    ...
    lastyear_complete_season = (
        hdates_pg[:, -mxharvests:] >= sdates_pg[:, -mxharvests:]
    ) | np.isinf(hdates_pg[:, -mxharvests:])
...
def convert_axis_time2gs_old(this_ds, myVars):
    ...
    if mxharvests < 2:
        raise RuntimeError(
            f"convert_axis_time2gs() assumes max harvests per year == 2, not {mxharvests}"
        )
    ...
    cond1 = this_ds["HDATES"].isel(time=0, mxharvests=0) < this_ds["SDATES"].isel(
        time=0, mxsowings=0
    )
    ...
    cond2 = np.bitwise_and(
        this_ds["HDATES"].isel(time=0, mxharvests=0) == this_ds["SDATES"].isel(time=0, mxsowings=0),
    ...
    if np.max(Nharvests_eachPatch) > Ngs + 1:
        raise ValueError(
            f"Expected at most {Ngs+1} harvests in any patch; found at least one patch with"
            f" {np.max(Nharvests_eachPatch)}"
        )
    h = mxharvests
    ...
    new_ds_gs = extract_gs_timeseries(
        new_ds_gs, "SDATES", this_ds["SDATES"], sdate_included, Npatches, Ngs + 1
    )
...
def convert_units(da, tgt_units):
    ...
    if da_units not in multiplier_dict.keys() or tgt_units not in multiplier_dict[da_units].keys():
        if (
            tgt_units not in multiplier_dict.keys()
            or da_units not in multiplier_dict[tgt_units].keys()
        ):
            raise RuntimeError(
                f"Conversion multiplier between {da_units} and {tgt_units} is not defined"
            )
...
def detrend(ps_in):
    ...
    if isinstance(ps_in, xr.DataArray):
        ps_out = xr.DataArray(data=ps_out, coords=ps_in.coords, attrs=ps_in.attrs)
...
def extract_gs_timeseries(this_ds, this_var, in_da, include_these, Npatches, Ngs):
    ...
    out_da = xr.DataArray(this_array, coords=this_ds.coords, dims=this_ds.dims)
...
def fao_data_get(fao_all, element, y1, yN, fao_to_clm_dict, cropList_combined_clm):
    ...
    if len(fao_to_clm_dict) != len(cropList_combined_clm):
        raise RuntimeError(
            f"fao_to_clm_dict and are different lengths ({len(cropList_combined_clm)} vs"
            f" {len(cropList_combined_clm)})"
        )
    if len(fao_this.columns) != len(cropList_combined_clm):
        raise RuntimeError(
            f"fao_this.columns and are different lengths ({len(fao_this.columns)} vs"
            f" {len(cropList_combined_clm)})"
        )
    ...
    if "diagnose" in which_cases:
        if ".2022" in which_cases:
            ...
            cases["Prescribed Sowing 22"] = {
                ...
                "constantGSs": (
                    None
                ),  # 'None' with constantVars specified means all should be constant
                "res": "f19_g17",
                "rx_sdates_file": "/Users/Shared/CESM_work/crop_dates/sdates_ggcmi_crop_calendar_phase3_v1.01_nninterp-f19_g17.2000-2000.20220727_164727.nc",
                "rx_hdates_file": None,
                "rx_gdds_file": None,
...
def get_earthstat_country_ts(
    earthstats, res, thisVar, countries_map, thisCrop_clm, i_theseYears_earthstat, country_id
):
    ...
    tmp = earthstats[res][thisVar].sel(crop=thisCrop_clm.title()).isel(time=i_theseYears_earthstat)
    if country_id != None:
    ...
    if np.any(result < 0):
        if np.any(result[result < 0] < -1e-24):
            raise RuntimeError(
                f"Negative value(s) in EarthStat country timeseries; min {np.min(result)}"
            )
        result[result < 0] = 0
...
def get_faostat_country_ts(fao_all_ctry, thisCrop_fao, top_y1, top_yN, country, element):
    tmp = fao_all_ctry.query(
        f'Crop == "{thisCrop_fao}" & Element == "{element}" & Year >= {top_y1} & Year <= {top_yN}'
    )
    if country == "World":
...
def get_diff_earthstat(case_ds, case, earthstats_ds, reses, use_absolute_bias=False):
    ...
    if attrs_differ_or_missing(case_ds, "YIELD_ANN_DIFFEARTHSTAT", attrs_tgt_dict):
        case_ds["YIELD_ANN_DIFFEARTHSTAT"] = convert_units(
            case_ds["YIELD_ANN"], "g/m2"
        ) - convert_units(earthstats_ds["Yield"], "g/m2")
        if use_absolute_bias:
            case_ds["YIELD_ANN_DIFFEARTHSTAT"] = np.fabs(case_ds["YIELD_ANN_DIFFEARTHSTAT"])
        case_ds["YIELD_ANN_DIFFEARTHSTAT"] = case_ds["YIELD_ANN_DIFFEARTHSTAT"].where(
            earthstats_ds["PhysicalArea"] > 0
        )
    ...
    if attrs_differ_or_missing(case_ds, "PROD_ANN_DIFFEARTHSTAT", attrs_tgt_dict):
        ...
        if yield_tmp.attrs["units"] != "g/m2":
            raise RuntimeError(
                f"Unknown units in YIELD_ANN_DIFFEARTHSTAT: {yield_tmp.attrs['units']}"
            )
...
def get_irrigation_use_relative_to_supply(cases):
    for casename, case in cases.items():
        ...
        case["ds"]["QIRRIG_FROM_SURFACE_GRID_PKMTH_ANN"] = xr.DataArray(
            ...
            coords={"time": case["ds"]["time"], "gridcell": case["ds"]["gridcell"]},
            attrs={"long_name": "Peak month of irrigation use", "units": "month"},
        )

        # Get VALUE of each gridcell-year's max monthly irrigation use
        case["ds"]["QIRRIG_FROM_SURFACE_GRID_VALPKMTH_ANN"] = (
            case["ds"]["QIRRIG_FROM_SURFACE_GRID_MTH"].groupby(case["ds"]["time_mth"].dt.year).max()
        )

        # Convert the above from mm/d to m3/d
        case["ds"]["IRRIG_FROM_SURFACE_GRID_VALPKMTH_ANN"] = (
            case["ds"]["QIRRIG_FROM_SURFACE_GRID_VALPKMTH_ANN"] * case["ds"]["AREA_GRID"] * 1e-3
        )
        ...
        irrig_supply_grid_valpkmthwithdrawal_ann = np.full_like(
            case["ds"]["QIRRIG_FROM_SURFACE_GRID_VALPKMTH_ANN"], np.nan
        )
        ...
        for y in np.arange(len(yearly_split)):
            ...
            thisYear_QIRRIG_FROM_SURFACE_GRID_PKMTH_g = case["ds"][
                "QIRRIG_FROM_SURFACE_GRID_PKMTH_ANN"
            ].isel(time=y)
            ...
            test_take_along_axis = np.squeeze(
                np.take_along_axis(
                    thisYear_IRRIG_SUPPLY_GRID_MTH_mg,
                    np.expand_dims(thisYear_QIRRIG_FROM_SURFACE_GRID_PKMTH_g, axis=0),
                    0,
                )
            )
        ...
        case["ds"]["IRRIG_SUPPLY_GRID_VALPKMTHWITHDRAWAL_ANN"] = xr.DataArray(
            ...
            attrs={
                "long_name": "Main channel river volume in month of peak irrigation demand",
                "units": "m3",
            },
        )
        ...
        case["ds"]["IRRIG_WITHDRAWAL_FRAC_SUPPLY_VALPKMTHWITHDRAWAL_ANN"] = xr.DataArray(
            data=case["ds"]["IRRIG_FROM_SURFACE_GRID_VALPKMTH_ANN"]
            / (48 * case["ds"]["IRRIG_SUPPLY_GRID_VALPKMTHWITHDRAWAL_ANN"]),
...
def get_mean_byCountry(fao, top_y1, top_yN):
    return (
        fao.query(f"Year>={top_y1} & Year<={top_yN}")
        .groupby(["Crop", "Element", "Area"])["Value"]
        .mean()
    )
...
def get_peakmonth(cases, this_var, y1=None, yN=None):
    ...
    for casename, case in cases.items():
        ...
        if "time_mth" not in ds[v].dims:
            raise RuntimeError(
                f"Requested peak month of {v} but 'time_mth' not in dims {ds[v].dims}"
            )
        ...
        # Save as DataArray
        ds[this_var] = xr.DataArray(
            data=pkmth,
            coords=ds[shapevar].coords,
            attrs={"units": "peak month", "y1": y1, "yN": yN},
        )
...
def get_timeseries_bias(sim, obs, weights=None):
    ...
    if sim.ndim > 2:
        raise RuntimeError(
            "Need to collapse all dimensions of sim after the first into the second."
        )
    if obs.ndim > 1:
...
def get_topN_ds(
    ...
    if earthstats_res not in earthstats:
        raise RuntimeError(
            "What EarthStat resolution should be used to get country timeseries, if not"
            f" {earthstats_res}?"
        )
    ...
    i_theseYears_earthstat = [
        i
        for i, x in enumerate(earthstats["f19_g17"].time.values)
        if (x.year >= plot_y1) and (x.year <= plot_yN)
    ]
    ...
    for i_case, (casename, case) in enumerate(cases.items()):
        ...
        i_theseYears_case = [
            i
            for i, x in enumerate(case_ds.time.values)
            if (x.year >= plot_y1) and (x.year <= plot_yN)
        ]
        i_theseYears_lu = [
            i
            for i, x in enumerate(lu_ds.time.values)
            if (x.year >= plot_y1) and (x.year <= plot_yN)
        ]

        # Find this crop in production and area data
        i_thisCrop_case = [
            i for i, x in enumerate(case_ds.patches1d_itype_veg_str.values) if thisCrop_clm in x
        ]
        if len(i_thisCrop_case) == 0:
            raise RuntimeError(f"No matches found for {thisCrop_fao} in case_ds.vegtype_str")
        i_thisCrop_lu = [
            i
            for i, x in enumerate(lu_ds.patches1d_itype_veg.values)
            if thisCrop_clm in utils.ivt_int2str(x)
        ]
        ...
        # Countries
        countries_da = (
            countries.isel(patch=i_thisCrop_lu)
            .groupby(tmp_ds["patches1d_gi"])
        ...
        for c, country in enumerate(topN_countries):
            if country == "World":
                ...
                if len(country_id) != 1:
                    if len(country_id) == 0:
                        continue
                    raise RuntimeError(
                        f"Expected 0 or 1 match of {country} in countries_key; got"
                        f" {len(country_id)}"
                    )
            ...
            if i_case == 0:
                prod_faostat_yc[:, c] = get_faostat_country_ts(
                    fao_all_ctry, thisCrop_fao, plot_y1, plot_yN, country, "Production"
                )
                area_faostat_yc[:, c] = get_faostat_country_ts(
                    fao_all_ctry, thisCrop_fao, plot_y1, plot_yN, country, "Area harvested"
                )

            # EarthStat
            if np.all(np.isnan(prod_earthstat_yc[:, c])):
                prod_earthstat_yc[:, c] = get_earthstat_country_ts(
                    earthstats,
                    earthstats_res,
                    "Production",
                    countries_map,
                ...
                area_earthstat_yc[:, c] = get_earthstat_country_ts(
                    earthstats,
                    earthstats_res,
                    "HarvestArea",
                    countries_map,
                ...
                if np.any((prod_earthstat_yc[:, c] > 0) & (area_earthstat_yc[:, c] == 0)):
                    raise RuntimeError(
                        f"EarthStat has 0 area but positive production for {country}"
                    )

    if np.any(area_ar == 0):
        raise RuntimeError("0 in area_ar")

    new_coords = {"Case": caselist, "Year": plotYears, "Country": topN_countries}
    prod_da = xr.DataArray(data=prod_ar, coords=new_coords, attrs={"units": "tons"})
    area_da = xr.DataArray(data=area_ar, coords=new_coords, attrs={"units": "ha"})
    yield_da = prod_da / area_da
    yield_da = yield_da.assign_attrs({"units": "tons/ha"})
    prod_faostat_da = xr.DataArray(
        data=prod_faostat_yc,
        coords={"Year": plotYears, "Country": topN_countries},
        attrs={"units": "tons"},
    )
    area_faostat_da = xr.DataArray(
        data=area_faostat_yc,
        coords={"Year": plotYears, "Country": topN_countries},
        attrs={"units": "ha"},
    )
    yield_faostat_da = prod_faostat_da / area_faostat_da
    yield_faostat_da = yield_faostat_da.assign_attrs({"units": "tons/ha"})
    prod_earthstat_da = xr.DataArray(
        data=prod_earthstat_yc,
        coords={"Year": plotYears, "Country": topN_countries},
    )
    area_earthstat_da = xr.DataArray(
        data=area_earthstat_yc, coords={"Year": plotYears, "Country": topN_countries}
    )
    ...
    for i, v in enumerate(topN_ds):
        ...
        if "Case" in topN_ds[v].dims:
            ...
            topN_dt_ds[v] = xr.DataArray(
                data=tmp_dt_cyC, coords=topN_ds[v].coords, attrs=topN_ds[v].attrs
            )
            for C, country in enumerate(topN_countries):
                tmp_ya_cy = tmp_ya_cyC[:, :, C]
                tmp_ya_cy = yield_anomalies(tmp_ya_cy)
                tmp_ya_cyC[:, :, C] = tmp_ya_cy
            topN_ya_ds[v] = xr.DataArray(
                data=tmp_ya_cyC, coords=topN_ds[v].coords, attrs=topN_ds[v].attrs
            )
        else:
            tmp_dt_Cy = np.transpose(topN_ds[v].copy().values)
            tmp_dt_Cy = detrend(tmp_dt_Cy)
            topN_dt_ds[v] = xr.DataArray(
                data=np.transpose(tmp_dt_Cy), coords=topN_ds[v].coords, attrs=topN_ds[v].attrs
            )
            tmp_ya_Cy = np.transpose(topN_ds[v].copy().values)
            tmp_ya_Cy = yield_anomalies(tmp_ya_Cy)
            topN_ya_ds[v] = xr.DataArray(
                data=np.transpose(tmp_ya_Cy), coords=topN_ds[v].coords, attrs=topN_ds[v].attrs
            )
    ...
    for c, thisCrop in enumerate(cropList_combined_clm[:-1]):
        ...
        for pft_str in yield_gd.ivt_str.values:
            ...
            if map_prod_thisCrop_clm.shape != map_yield_thisCrop_clm.shape:
                raise RuntimeError(
                    f"Error getting production map {map_yield_thisCrop_clm.dims}: expected shape"
                    f" {map_yield_thisCrop_clm.shape} but got {map_prod_thisCrop_clm.shape}"
                )
...
def get_ts_prod_clm_yc_da2(case_ds, lu_ds, yieldVar, cropList_combined_clm, quiet=False):
    ...
    if len(non_patch_dims) != 1:
        raise RuntimeError(
            f"Expected one non-patch dimension of case_ds['{yieldVar}']; found"
            f" {len(non_patch_dims)}: {non_patch_dims}"
        )
    ...
    if time_dim_in != "time":
        if not quiet:
            print(
                "WARNING: Using calendar years from LU data with yield data of time dimension"
                f" {time_dim_in}."
            )
        lu_ds = lu_ds.assign_coords({"time": case_ds[yieldVar].gs.values}).rename(
            {"time": time_dim_in}
        )
    ...
    ts_prod_clm_yc_da = (
        prod_da.groupby(case_ds["patches1d_itype_combinedCropCLM_str"])
        .apply(xr.DataArray.sum, dim="patch", skipna=True)
        .rename({time_dim_in: time_dim_out, "patches1d_itype_combinedCropCLM_str": "Crop"})
...
def get_yield(ds, **kwargs):
    ...
    for key, selection in kwargs.items():
        if key == "out_var" and selection != "YIELD":
            raise RuntimeError(
                "get_yield() may only be called with out_var='YIELD'. Did you mean to call"
                " zero_immatures() instead?"
            )
...
def get_yield_ann(ds, min_viable_hui=1.0, mxmats=None, force_update=False, lu_ds=None):
    ...
    def do_calculate(thisVar, ds, force_update, min_viable_hui, mxmat_limited):
        ...
        if consider_skipping:
            already_calculated = (
                ds[f"{thisVar}_ANN"].attrs["min_viable_hui"] == min_viable_hui
                and ds[f"{thisVar}_ANN"].attrs["mxmat_limited"] == mxmat_limited
            )
            is_locked = (
                "locked" in ds[f"{thisVar}_ANN"].attrs and ds[f"{thisVar}_ANN"].attrs["locked"]
            )
    ...
    if do_calculate("YIELD", ds, force_update, min_viable_hui, mxmat_limited):
        ...
        ds["YIELD_ANN"] = xr.DataArray(
            data=tmp, attrs=grainc_to_food_ann_orig.attrs, coords=grainc_to_food_ann_orig.coords
    ...
    if lu_ds is not None and do_calculate("PROD", ds, force_update, min_viable_hui, mxmat_limited):
        if lu_ds["AREA_CFT"].attrs["units"] != "m2":
            raise RuntimeError(
                f"Unknown units of lu_ds['AREA_CFT']: {lu_ds['AREA_CFT'].attrs['units']}"
            )
        if ds["YIELD_ANN"].attrs["units"] != "g/m2":
            raise RuntimeError(
                f"Unknown units of ds['YIELD_ANN']: {ds['YIELD_ANN'].attrs['units']}"
            )
        ...
        for v in this_ds:
            ...
            if the_min < 0:
                if np.abs(the_min) <= tiny_negOK:
                    if verbose:
                        print(
                            f"Tiny negative value(s) in {v} (abs <= {tiny_negOK}) being set to 0"
                            f" ({which_file})"
                        )
                else:
                    print(
                        f"WARNING: Unexpected negative value(s) in {v}; minimum"
                        f" {the_min} ({which_file})"
                    )
...
def mask_immature(this_ds, this_vegtype, gridded_da):
    ...
    reason_gridded = utils.grid_one_variable(this_ds, thisVar, vegtype=this_vegtype).squeeze(
        drop=True
    )
...
def print_gs_table(ds):
    ...
    for v in ["gs", "SDATES", "GDDHARV", "HDATES", "HARVEST_REASON"]:
        ...
        data[v] = ds[v].values
    print(
        f"Lon {ds.patches1d_lon.values} lat {ds.patches1d_lat.values},"
        f" {ds.patches1d_itype_veg_str.values}"
    )
...
def round_lonlats_to_match_da(ds_a, varname_a, tolerance, ds_b=None, varname_b=None):
    ...
    if max_diff > tolerance:
        ...
        if new_tolerance > 0:
            ...
            print(diffs)
            raise RuntimeError(
                f"Maximum disagreement in {varname_a} ({max_diff}) is intolerably large"
            )
        new_tolerance = 10**new_tolerance
        if varname_b:
            print(
                f"Maximum disagreement between {varname_a} and {varname_b} ({max_diff}) is larger"
                f" than default tolerance of {tolerance}. Increasing tolerance to {new_tolerance}."
            )
        else:
            print(
                f"Maximum disagreement in {varname_a} ({max_diff}) is larger than default tolerance"
                f" of {tolerance}. Increasing tolerance to {new_tolerance}."
            )
    ...
    def replace_vals(ds, new_da, varname):
        if len(ds[varname]) != len(new_da):
            raise RuntimeError(
                f"Not allowing you to change length of {varname} from {len(ds[varname])} to"
                f" {len(new_da)}"
            )
        if varname in ds.coords:
            ...
            raise TypeError(
                f"{varname} is not a coordinate or data variable; update"
                " round_lonlats_to_match_da()->replace_vals() to handle it"
            )
        return ds
...
def round_lonlats_to_match_ds(ds_a, ds_b, which_coord, tolerance):
    ...
    while tolerance != tolerance_orig:
        if i > max_Nloops:
            raise RuntimeError(
                f"More than {max_Nloops} loops required in round_lonlats_to_match_ds()"
            )
        tolerance_orig = tolerance
        patches_var = "patches1d_" + which_coord
        if patches_var in ds_a and which_coord in ds_a:
            ds_a, tolerance = round_lonlats_to_match_da(
                ds_a, patches_var, tolerance, varname_b=which_coord
            )
        if patches_var in ds_b and which_coord in ds_b:
            ds_b, tolerance = round_lonlats_to_match_da(
                ds_b, patches_var, tolerance, varname_b=which_coord
            )
        if patches_var in ds_a and patches_var in ds_b:
            ds_a, ds_b, tolerance = round_lonlats_to_match_da(
                ds_a, patches_var, tolerance, ds_b=ds_b
            )
        if which_coord in ds_a and which_coord in ds_b:
            ds_a, ds_b, tolerance = round_lonlats_to_match_da(
                ds_a, which_coord, tolerance, ds_b=ds_b
            )
    return ds_a, ds_b, tolerance
...
def subtract_mean(in_ps):
    ...
    )  # Happens when you do np.nanmean() of an all-NaN
...
def time_units_and_trim(ds, y1, yN, dt_type):
    ...
    if not isinstance(time0, cftime.datetime):
        ...
        print(
            f"Converting integer time to {dt_type}, assuming values are years and each time step is"
            " Jan. 1."
        )
...
def ungrid(gridded_xr, ungridded_target_ds, coords_var=None, target_dim="patch", **kwargs):
    ...
    for key, selection in kwargs.items():
        if key in ["lon", "lat"]:
            isel_dict[key] = xr.DataArray(
                [int(x) - 1 for x in ungridded_target_ds[selection].values], dims=target_dim
            )
        else:
            values_list = list(gridded_xr[key].values)
            isel_dict[key] = xr.DataArray(
                [values_list.index(x) for x in ungridded_target_ds[selection].values],
                dims=target_dim,
            )
...
def zero_immatures(
    ds, out_var="YIELD", min_viable_hui=None, mxmats=None, forAnnual=False, force_update=False
):
    mxmat_limited = bool(mxmats)
    ...
    if (
        ...
        and all([x in ds[out_var].attrs for x in ["min_viable_hui", "mxmat_limited"]])
    ):
        if (
            ds[out_var].attrs["min_viable_hui"] == min_viable_hui
            and ds[out_var].attrs["mxmat_limited"] == mxmat_limited
        ):
            return ds
    ...
    if min_viable_hui is not None:
        ...
        if np.any(huifrac < min_viable_hui_touse):
            print(
                f"Setting {out_var} to zero where minimum viable HUI ({min_viable_hui}) wasn't"
                " reached"
            )
            ...
            ds[out_var] = xr.DataArray(data=tmp, attrs=tmp_da.attrs, coords=tmp_da.coords)
    ...
    if mxmat_limited:
        ...
        ds[out_var] = xr.DataArray(data=tmp_ra, coords=tmp_da.coords, attrs=tmp_da.attrs)
