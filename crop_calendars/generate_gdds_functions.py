...
def import_rx_dates(s_or_h, date_inFile, incl_patches1d_itype_veg, mxsowings, logger):
    if isinstance(date_inFile, xr.Dataset):
        ...
        error(
            logger,
            (
                f"Importing {s_or_h}dates_rx: Expected date_inFile to be str or DataArray, not"
                f" {type(date_inFile)}"
            ),
        )
    ...
    if y == 0:
        ...
        if incl_vegtypes_str != list(dates_incl_ds.vegtype_str.values):
            error(
                logger,
                (
                    f"Included veg types differ. Previously {incl_vegtypes_str}, now"
                    f" {dates_incl_ds.vegtype_str.values}"
                ),
            )
        ...
        if incorrectly_daily and list(unique_hdates) == [364]:
            log(
                ...
                (
                    ...
                    " might have just been because of incorrectly daily outputs. Setting them to"
                    " 365."
                ),
            )
            ...
            error(
                logger,
                (
                    f"{len(skip_patches_for_isel_sdatelt1)} patches have SDATE < 1. Unique affected"
                    f" hdates: {unique_hdates}"
                ),
            )
    ...
    if incl_thisyr_but_nan_lastyr and list(skip_patches_for_isel_hdatelt1):
        ...
        if np.any(hdates_thisyr_where_nan_lastyr < 1):
            ...
            log(
                logger,
                (
                    f"   ❗ {len(here)} patches have harvest date -1 because they weren't active"
                    " last year (and were either never active or were harvested when last active)."
                    " Ignoring, but you should have done a run with patches always active if they"
                    " are ever active in the real LU timeseries."
                ),
            )
    ...
    if skipping_patches_for_isel_hdatelt1:
        ...
        if incorrectly_daily and list(unique_sdates) == [1]:
            log(
                logger,
                (
                    f"   ❗ {len(skip_patches_for_isel_hdatelt1)} patches have HDATE < 1??? Seems"
                    " like this might have just been because of incorrectly daily outputs; setting"
                    " them to 365."
                ),
            )
            ...
            error(
                logger,
                (
                    f"{len(skip_patches_for_isel_hdatelt1)} patches have HDATE < 1. Possible"
                    " causes:\n   * Not using constant crop areas (e.g., flanduse_timeseries from"
                    " make_lu_for_gddgen.py)\n   * Not skipping the first 2 years of"
    ...
    if mxmats and (imported_sdates or imported_hdates):
        ...
        for v in hdates_rx_orig:
            ...
            print(
                ...
                f" {int(np.max(get_gs_len_da(hdates_rx[v] - sdates_rx[v]).values))}."
            )
    else:
    ...
    if "GDDHARV" not in h1_ds:
        ...
        try:
            ...
            h3_ds = utils.import_ds(
                glob.glob(pattern), myVars=["GDDHARV"], myVegtypes=utils.define_mgdcrop_list()
            )
    ...
    for v, vegtype_str in enumerate(h1_incl_ds.vegtype_str.values):
        ...
        if np.any(np.isnan(gddaccum_atharv_p)):
            log(
                ...
                (
                    "         ❗"
                    f" {np.sum(np.isnan(gddaccum_atharv_p))}/{len(gddaccum_atharv_p)} NaN after"
                    " extracting GDDs accumulated at harvest"
                ),
            )
        if save_figs and np.any(np.isnan(gddharv_atharv_p)):
            log(
                logger,
                (
                    f"         ❗ {np.sum(np.isnan(gddharv_atharv_p))}/{len(gddharv_atharv_p)} NaN"
                    " after extracting GDDHARV"
                ),
            )
        ...
        if y > 0:
            ...
            if save_figs:
                gddharv_yp_list[v][y - 1, active_thisYear_where_gs_lastyr_indices] = (
            ...
            if np.any(
                ...
                if incorrectly_daily:
                    log(
                        logger,
                        (
                            "         ❗ Unexpected NaN for last season's GDD accumulation."
                            " Allowing because this might just be an artifact of incorrectly daily"
                            " outputs, BUT RESULTS MUST NOT BE TRUSTED."
                        ),
                    )
            ...
            if not np.array_equal(nanmask_output_gdds_lastyr, nanmask_output_sdates):
                if incorrectly_daily:
                    log(
                        logger,
                        (
                            "         ❗ NaN masks differ between this year's sdates and"
                            " 'filled-out' GDDs from last year. Allowing because this might just be"
                            " an artifact of incorrectly daily outputs, BUT RESULTS MUST NOT BE"
                            " TRUSTED."
                        ),
                    )
                else:
