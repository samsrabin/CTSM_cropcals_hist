...
def chunk_colorbar(
    ...
    if "maskcolorbar_near0" in varInfo and varInfo["maskcolorbar_near0"][v] is not None:
        ...
        if isinstance(nearzero_thresh, str):
            ...
            if len(nearzero_parts) > 2:
                if nearzero_parts[2] != "cumulative":
                    raise RuntimeError(
                        f"Unable to parse maskcolorbar_near0 value {nearzero_thresh}"
                    )
                frac_to_include = 1 - nearzero_val_in / 100
                nearzero_thresh = get_threshold_lowestpercentile_cumulative(
                    this_map, frac_to_include
                )
            else:
        ...
        # Add near-zero bin
        bounds, cbar_spacing, pct_absdiffs_masked_before, ticklabels, ticklocations, vmax, vmin = (
            maskcolorbar_near0(
                binwidth,
                bounds,
                cbar_spacing,
        ...
        if underlay is not None:
            raise RuntimeError(
                "You need a different color to distinguish maskcolorbar_near0 cells from"
                " other-masked cells"
            )
        ...
        if posNeg:
            if crop == "Crops decreasing":
                new_colors = np.concatenate(
                    (this_cmap.colors[: int(Nbins / 2)], np.array([cropcal_colors["underlay"]])),
                    axis=0,
                )
...
def get_amount_masked(
    crop, this_map_timemean, sumdiff_beforemask, pct_absdiffs_masked_before, reason
):
...
def get_non_rx_map(
    var_info, cases, casename, this_var, thisCrop_main, found_types, plot_y1, plot_yN, ref_casename
):
    ...
    if "gs" in this_map.dims:
        ...
        raise RuntimeError(
            f"Unsure how to mask patch-years with no area for {this_var} with dims {this_map.dims}"
        )
    ...
    if "lon" not in this_map.dims:
        ...
        croparea_ever_positive = (
            utils.grid_one_variable(this_ds, "croparea_ever_positive", vegtype=found_types) > 0
        )
...
def maskcolorbar_near0(
    ...
    if posNeg:
        if crop == "Crops decreasing":
            ...
            bounds = np.concatenate(
                (np.arange(vmin, -binwidth + 1e-9, binwidth), np.array([-nearzero_thresh, 0]))
            )
            if not use_proportional_cbar:
                ticklabels = np.concatenate(
                    (
                ...
                ticklocations = np.concatenate(
                    (ticklocations[ticklocations < 0], [-nearzero_thresh, 0])
                )
        elif crop == "Crops increasing":
            vmin = 0
            ticklocations = ticklocations[np.where(ticklocations >= 0)]
            bounds = np.concatenate(
                (np.array([0, nearzero_thresh]), np.arange(binwidth, vmax + 1e-9, binwidth))
            )
            if not use_proportional_cbar:
                ticklabels = np.concatenate(
                    (
                ...
                ticklocations = np.concatenate(
                    ([0, nearzero_thresh], ticklocations[ticklocations > 0])
                )
        else:
        ...
        bounds = np.concatenate(
            (
                np.arange(vmin, -binwidth + 1e-9, binwidth),
    ...
    # Diagnostics
    pct_absdiffs_masked_before = get_amount_masked(
        crop,
        this_map_timemean_fake,
        sumdiff_beforemask,
        pct_absdiffs_masked_before,
        f"nearest to zero, threshold {nearzero_thresh}",
    )
