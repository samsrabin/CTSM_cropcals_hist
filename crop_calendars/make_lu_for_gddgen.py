...
ds_in['AREA_CROP'] = (ds_in.AREA * ds_in.LANDFRAC_PFT * ds_in.PCT_CROP/100).transpose("time", "lsmlat", "lsmlon")
ds_in['AREA_CFT'] = (ds_in['AREA_CROP'] * ds_in.PCT_CFT/100).transpose("time", "cft", "lsmlat", "lsmlon")
ds_area_max = ds_in['AREA_CFT'].max(dim="time")
ds_in['ever_active_bycft'] = (ds_area_max > 0).transpose("cft", "lsmlat", "lsmlon")
ds_in['ever_active'] = ds_in['ever_active_bycft'].any(dim="cft")
...
# Save output netCDF
ds_out = xr.concat((ds_in.sel(time=slice(0, first_fake_year-1)),
                    ds_tmp), dim="time")
