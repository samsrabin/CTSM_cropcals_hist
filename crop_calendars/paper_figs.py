...
import pandas as pd
import cftime
from fig_global_timeseries import (
    global_timeseries_yieldetc,
    global_timeseries_irrig_inclcrops,
    global_timeseries_irrig_allcrops,
)
...
warnings.filterwarnings(
    ...
    message=(
        ...
        " the length of the `geoms` property instead to get the  number of parts of a multi-part"
        " geometry."
    ),
)
warnings.filterwarnings(
    "ignore",
    message=(
        "Iteration over multi-part geometries is deprecated and will be removed in Shapely 2.0. Use"
        " the `geoms` property to access the constituent parts of a multi-part geometry."
    ),
)
...
warnings.filterwarnings(
    "ignore",
    message=(
        "This figure includes Axes that are not compatible with tight_layout, so results might be"
        " incorrect."
    ),
)
...
for i, (casename, case) in enumerate(cases.items()):
    ...
    if casename == "cmip6":
        ...
        this_ds = this_ds.assign_coords(
            {
                "ivt": np.arange(
                    np.min(this_ds.patches1d_itype_veg.values),
        ...
        # Add vegtype_str
        this_ds["vegtype_str"] = xr.DataArray(
            data=[utils.ivt_int2str(x) for x in this_ds.ivt.values], dims={"ivt": this_ds.ivt}
        )
        ...
        bad_patches = cc.check_constant_vars(
            this_ds,
            case,
            ignore_nan=True,
            constantGSs=case["constantGSs"],
            verbose=True,
            throw_error=False,
        )
...
for casename, case in cases.items():
    ...
    ivt_str_ever_active = np.unique(
        case_ds.isel(patch=ever_active)["patches1d_itype_veg_str"].values
    )
...
for _, case in cases.items():
    case["ds"]["patches1d_itype_combinedCropCLM_str"] = xr.DataArray(
        cc.fullname_to_combinedCrop(
            case["ds"]["patches1d_itype_veg_str"].values, cropList_combined_clm
        ),
...
# Define resolutions
reses = {}
lu_dir = "/Users/Shared/CESM_work/CropEvalData_ssr"
# f09_g17 ("""1-degree"""; i.e., 1.25 lon x 0.9 lat)
reses["f09_g17"] = {
    "lu_path": os.path.join(
        lu_dir, "landuse.timeseries_0.9x1.25_hist_78pfts_CMIP6_simyr1850-2015_c170824.nc4"
    )
}
# f19_g17 ("""2-degree"""; i.e., 2.5 lon x 1.9 lat), ctsm5.0
reses["f19_g17"] = {
    "lu_path": os.path.join(
        lu_dir, "landuse.timeseries_1.9x2.5_hist_78pfts_CMIP6_simyr1850-2015_c170824.nc4"
    )
}
# f19_g17 ("""2-degree"""; i.e., 2.5 lon x 1.9 lat), ctsm5.2
reses["f19_g17_ctsm5.2"] = {
    "lu_path": os.path.join(
...
for i, (resname, res) in enumerate(reses.items()):
    ...
    res["ds"] = res["ds"].assign_coords(
        {
            "time": [
                cftime.DatetimeNoLeap(y, 1, 1, 0, 0, 0, 0, has_year_zero=True)
                for y in res["ds"].time.values
            ]
        }
    )
...
for i, (casename, case) in enumerate(cases.items()):
    ...
    case_ds, lu_ds, lon_tolerance = cc.round_lonlats_to_match_ds(
        case_ds, lu_ds, "lon", initial_tolerance
    )
    case_ds, lu_ds, lat_tolerance = cc.round_lonlats_to_match_ds(
        case_ds, lu_ds, "lat", initial_tolerance
    )
...
for i, (casename, case) in enumerate(cases.items()):
    ...
    croparea_positive_wholeseason[where_harvyear_not_sowyear] = croparea_positive_bothyears[
        where_harvyear_not_sowyear
    ]
    # Convert that to DataArray, using time coordinate of year Y as growing season
    case["ds"]["croparea_positive_wholeseason"] = xr.DataArray(
        data=croparea_positive_wholeseason, coords={"patch": lu_ds["patch"], "gs": yearY_as_gs}
    )
    ...
    for i, (casename, case) in enumerate(cases.items()):
        ...
        for v in case["ds"]:
            ...
            if "PATCH" in v2:
                ...
                case["ds"][v3] = (
                    case["ds"][v2]
                    .groupby(case["ds"][grid_index_var])
                    .sum()
                    .rename({grid_index_var: "grid"})
                )
    ...
    if "rx_sdates_file" in case:
            ...
            if v in case:
                for l in ["lon", "lat"]:
                    ...
                    if max_diff_orig > lonlat_tol:
                        raise RuntimeError(
                            f"{v} {l} values differ too much from {casename} ({max_diff_orig} >"
                            f" {lonlat_tol})"
                        )
        ...
        if case["rx_sdates_file"] and case["rx_hdates_file"]:
            ...
            for v in case["rx_gslen_ds"]:
                if v == "time_bounds":
                    continue
                case["rx_gslen_ds"][v] = cc.get_gs_len_da(
                    case["rx_hdates_ds"][v] - case["rx_sdates_ds"][v]
                )
...
fao_prod, fao_prod_nosgc = cc.fao_data_get(
    fao_all, "Production", y1, yN, fao_to_clm_dict, cropList_combined_clm
)
fao_area, fao_area_nosgc = cc.fao_data_get(
...
earthstats_gd["f09_g17"] = (
    earthstats_gd["f09_g17"]
    .isel(crop=[cropList_fao_gd_all.index(x) for x in cropList_fao_gd])
    .assign_coords({"crop": cropList_combined_clm[:-1]})
)

# Include just years we care about
earthstats_gd["f09_g17"] = earthstats_gd["f09_g17"].isel(
    time=[
        i
        for i, x in enumerate(earthstats_gd["f09_g17"].time.values)
        if x.year in np.arange(y1, yN + 1)
    ]
)

# Interpolate to lower res, starting with just variables that won't get messed up by the
# interpolation
...
earthstats_gd["f19_g17"] = (
    earthstats_gd["f09_g17"].drop(labels=drop_vars).interp(lon=interp_lons, lat=interp_lats)
)
...
for v in earthstats_gd["f19_g17"]:
    ...
    vals[vals < 0] = 0
    earthstats_gd["f19_g17"][v] = xr.DataArray(
        data=vals,
...
recalc_ds = xr.Dataset(
    data_vars={
        ...
        "Production": (
            f19_g17_croparea_ha
            * earthstats_gd["f19_g17"]["HarvestFraction"]
            * earthstats_gd["f19_g17"]["Yield"]
        ),
    }
)
for v in recalc_ds:
    ...
    discrep_sum_rel = (
        100
        * (np.nansum(recalc_ds[v].values) - np.sum(earthstats_gd["f09_g17"][v].values))
        / np.sum(earthstats_gd["f09_g17"][v].values)
    )
...
for v in earthstats_gd["f19_g17"]:
    ...
    discrep_sum_rel = (
        100
        * (
            np.mean(earthstats_gd["f19_g17"][v].values)
            - np.mean(earthstats_gd["f09_g17"][v].values)
        )
        / np.mean(earthstats_gd["f09_g17"][v].values)
    )
    print(f"Discrepancy in {v} f19_g17 rel to f09_g17: {discrep_sum_rel}%")
...
importlib.reload(cc)
earthstats = {}
earthstats["f19_g17"] = cc.ungrid(
    earthstats_gd["f19_g17"],
    cases[[x for x in cases.keys()][0]]["ds"],
    "GRAINC_TO_FOOD_ANN",
    lon="patches1d_ixy",
    lat="patches1d_jxy",
...
for gridspec, dsg in earthstats_gd.items():
    dsg["countries"] = utils.lon_idl2pm(countries).interp_like(dsg["Area"], method="nearest")[
        "gadm0"
    ]
    if gridspec in earthstats:
        ds = earthstats[gridspec]
        ds["countries"] = cc.ungrid(
            dsg["countries"], ds, "Area", lon="patches1d_ixy", lat="patches1d_jxy"
        )
...
for resname, res in reses.items():
    if "dsg" not in res:
        continue
    res["dsg"]["countries"] = utils.lon_idl2pm(countries).interp_like(
        res["dsg"]["AREA"], method="nearest"
    )["gadm0"]
    res["ds"]["countries"] = cc.ungrid(
        res["dsg"]["countries"], res["ds"], "AREA", lon="patches1d_ixy", lat="patches1d_jxy"
    )
...
countries_key = countries_key.replace({"Turkey": "Türkiye", "Vietnam": "Viet Nam"})
# Replace some countries' names in FAO data to match key (to keep them short for figs)
fao_all_ctry = fao_all_ctry.replace(
    {
...
countries_key = countries_key.replace(
    {
        "China": "China, mld.",  # Because it also has Taiwan
        "United States": "USA",
...
cases = global_timeseries_yieldetc(
    cases,
    cropList_combined_clm,
    earthstats_gd,
    fao_area,
    fao_area_nosgc,
    fao_prod,
    ...
    reses,
    yearList,
    equalize_scatter_axes=equalize_scatter_axes,
    extra=extra,
    figsize=figsize,
    include_scatter=include_scatter,
    include_shiftsens=include_shiftsens,
    min_viable_hui_list=min_viable_hui,
    mxmats=mxmats_tmp,
    noFigs=noFigs,
    ny=ny,
    nx=nx,
    obs_for_fig=obs_for_fig,
    plot_y1=plot_y1,
    plot_yN=plot_yN,
    remove_scatter_bias=remove_scatter_bias,
    bias_round=bias_round,
    corrcoef_round=corrcoef_round,
    use_annual_yields=use_annual_yields,
    w=w,
)
...
if incl_irrig:
    ...
    global_timeseries_irrig_inclcrops(
        "IRRIG_APPLIED_PATCH_ANN",
        cases,
        reses,
        cropList_combined_clm,
        outDir_figs,
        extra="Total (all land)",
        figsize=figsize,
        noFigs=noFigs,
        ny=ny,
        nx=nx,
...
varList = {
    "GDDHARV": {"suptitle": "Mean harvest requirement", "time_dim": "gs", "units": "GDD"},
    "YIELD_ANN": {"suptitle": "Mean annual yield", "time_dim": "time", "units": "t/ha"},
    "PROD_ANN": {"suptitle": "Mean annual production", "time_dim": "time", "units": "Mt"},
    "GSLEN": {
    ...
    "HUI": {"suptitle": "Mean HUI at harvest", "time_dim": "gs", "units": "GDD"},
    "HUIFRAC": {
    ...
    "MATURE": {"suptitle": "Mature harvests", "time_dim": "gs", "units": "fraction"},
    "IRRIG_DEMAND_PATCH_ANN": {
...
for i, (casename, case) in enumerate(cases.items()):
    if i == 0:
        ...
        reason_list = np.unique(
            np.concatenate((reason_list, np.unique(case["ds"][thisVar].values)))
        )
...
for v, vegtype_str in enumerate(clm_types_rfir):
    ...
    for i, (casename, case) in enumerate(cases.items()):
        # Grid
        thisCrop_gridded = utils.grid_one_variable(
            case["ds"].sel(gs=slice(plot_gs1, plot_gsN)), thisVar, vegtype=vegtype_int
        ).squeeze(drop=True)
        ...
        for f, reason in enumerate(reason_list):
            ...
            axes.append(ax)
            im0 = make_map(
                ax, map_yx, fontsize, cmap=cmap, bounds=bounds, extend_bounds=extend, linewidth=0.3
            )
    ...
    for i, a in enumerate(topmost):
        axes[a].set_title(f"{reason_list_text[i]}", fontsize=fontsize["titles"], y=1.05)
...
for c, thisCrop in enumerate(fao_crops):
    ...
    if mxmat_limited:
        ...
        mxmats_tmp = None
    topN_ds, topN_dt_ds, topN_anom_ds = cc.get_topN_ds(
        cases,
        reses,
        topYears,
        Ntop,
    ...
    for c, country in enumerate(plot_ds.Country.values):
        ...
        ax = axes[c]
        sc = xr.plot.scatter(
            plot_ds.sel(Country=country), x="Yield (FAOSTAT)", y="Yield", hue="Case", ax=ax
        )
    ...
    for i, a in enumerate(bottommost):
        ...
        ax.text(
            x=x, y=y, s=f"{which_to_plot} (FAOSTAT)", fontsize=12, ha="center", fontweight="bold"
        )

    # Add figure title
    suptitle_xpos = 0.5
    f.suptitle(suptitle, x=suptitle_xpos, y=suptitle_ypos, fontsize=18, fontweight="bold")
    ...
    f.savefig(fig_outfile, bbox_inches="tight", facecolor="white")
    plt.close()
...
for thisCrop in fao_crops:
    ...
    file_prefix = which_to_plot.replace("aly", "")
    fig_outfile = (
        outDir_figs + f"{file_prefix} timeseries {plot_y1}-{plot_yN} top 10 " + suptitle + ".pdf"
    )
    if os.path.exists(fig_outfile) and not overwrite:
        print(f"    Skipping {thisCrop} (file exists).")
        continue
    ...
    # Get yield datasets
    print("    Analyzing...")
    topN_ds, topN_dt_ds, topN_ya_ds = cc.get_topN_ds(
        cases,
        reses,
        plotYears,
        topYears,
    ...
    for c, country in enumerate(plot_ds.Country.values):
        ...
        ax = axes[c]
        xr.plot.line(plot_ds[f"{thisVar} ({ref_dataset})"].sel(Country=country), "k--", ax=ax)

        for ca, case in enumerate(caselist):
            color = cropcal_colors_cases(case)
            if color is None:
                color = cm.Dark2(ca)
            xr.plot.line(plot_ds[thisVar].sel(Country=country, Case=case), color=color, ax=ax)
...
def get_country_data(ds, clm_crop, countries_thiscrop):
    da_thiscrop = (
        ds.sel(time=slice("1980-01-01", "2009-01-01"))
...
fao80_meanProd_byCountry = cc.get_mean_byCountry(
    fao_all_ctry[fao_all_ctry["Element"] == "Production"], 1980, 2009
)
fao80_meanArea_byCountry = cc.get_mean_byCountry(
    fao_all_ctry[fao_all_ctry["Element"] == "Area harvested"], 1980, 2009
)
fao92_meanProd_byCountry = cc.get_mean_byCountry(
    fao_all_ctry[fao_all_ctry["Element"] == "Production"], 1992, 2009
)
fao92_meanArea_byCountry = cc.get_mean_byCountry(
    fao_all_ctry[fao_all_ctry["Element"] == "Area harvested"], 1992, 2009
)
...
for c, fao_crop in enumerate(fao_crops):
    ...
    countries_thiscrop = lu_ds["countries"].isel(
        patch=np.where(case_ds["patches1d_itype_combinedCropCLM_str"] == clm_crop)[0]
    )
    ...
    countries_topNneg = [
        countries_key.name.values[countries_key.num == x][0] for x in countries_topNneg_int
    ]
    countries_topNpos = [
        countries_key.name.values[countries_key.num == x][0] for x in countries_topNpos_int
    ]
    topNcountries_int = np.flip(
        np.concatenate((countries_topNpos_int, np.flip(countries_topNneg_int)))
    )
    ...
    # Make plot
    ax_simProdDiff.barh(np.arange(2 * topN), xdata, tick_label=ylabels)
    ax_simProdDiff.set_title(clm_crop, weight="bold")
    ax_simProdDiff.set_xlabel(units)
    fig_simProdDiff.suptitle(
        "Production (1980-2009):\nPrescribed Calendars minus CLM Default", y=0.94, weight="bold"
    )
    ...
    # Make plot
    ax_absBiasDiff.barh(np.arange(2 * topN), absBiasDiff, tick_label=ylabels)
    ax_absBiasDiff.set_title(clm_crop, weight="bold")
    ax_absBiasDiff.set_xlabel(units)
    fig_absBiasDiff.suptitle(
        "Absolute yield bias (1980-2009):\nPrescribed Calendars minus CLM Default",
        y=0.94,
        weight="bold",
    )
    plt.subplots_adjust(wspace=wspace, hspace=hspace)
    ...
    fao92_meanProd_byCountry_thisCrop_topN = fao92_meanProd_byCountry_thisCrop.nlargest(
        topN, columns="Value"
    )
    fao92_meanProd_byCountry_thisCrop_other = fao92_meanProd_byCountry_thisCrop.nlargest(
        topN, columns="Value"
    )
    ...
    ax_faoProd.barh(np.arange(topN), xdata, tick_label=ylabels)
    ax_faoProd.set_title(clm_crop, weight="bold")
    ax_faoProd.set_xlabel(units)
    fig_faoProd.suptitle("Production (1980-2009): FAOSTAT", y=0.94, weight="bold")
...
for this_var, var_info in varList.items():
    ...
    for i, (casename, case) in enumerate(cases.items()):
        ...
        weights_time = xr.DataArray(
            data=np.full_like(case["ds"]["patches1d_lon"], 1 / Nyears),
            coords=case["ds"]["patches1d_lon"].coords,
        )
        if var_info["operation"] == "area-weighted mean":
            weight_numerators_time = reses[case["res"]]["ds"]["AREA_CFT"].sel(
                time=slice(f"{stat_y1}-01-01", f"{stat_yN}-12-31")
            )
        ...
        if thisCrop_str is not None:
            if "patch" not in case_da.dims:
                raise RuntimeError(
                    f"You can't distinguish crops in a DataArray with dims {case_da.dims}"
                )
            is_thisCrop = [
                True if thisCrop_str.lower() in x.lower() else False
                for x in case["ds"]["patches1d_itype_combinedCropCLM_str"].values
            ]
...
for thisVar_orig in varList:
    ...
    if Nggcmi_models_orig + len(cases) + 2 > ny * nx:
        raise RuntimeError(
            f"2 + {Nggcmi_models_orig} GGCMI models + CLM cases ({len(cases)}) > ny*nx ({ny*nx})"
        )
    vmin = 0.0
    ...
    for v, vegtype_str in enumerate(clm_sim_veg_types):
        ...
        for g, thisModel in enumerate(ggcmi_models_orig):
            ...
            if thisMax > 10**19:
                if verbose:
                    print(
                        f"Warning: {ncvar}: {thisModel}: Max {thisMax} (before filtering); setting"
                        " values >1e19 to NaN"
                    )
            ...
            if highMax:
                ...
                if thisMax > 366:
                    if verbose:
                        print(
                            f"Warning: {ncvar}: {thisModel}: Max {thisMax} {filter_str}; setting"
                            " values > 364 to NaN"
                        )
            ...
            if thisMin < 0:
                if verbose:
                    print(
                        f"{thisModel}: {ncvar}: Setting negative matyday values (min = {thisMin})"
                        " to NaN"
                    )
        ...
        if did_read:
            ...
            ggcmiDS[f"{ncvar}-inclmodels"] = matyday_da = xr.DataArray(
                data=ggcmi_models_bool, coords={"model": ggcmiDS.coords["model"]}
            )
        ...
        thisFile = os.path.join(
            ggcmi_cropcal_dir,
            f"{vegtype_str_ggcmi}_{tmp_rfir_token}_ggcmi_crop_calendar_phase3_v1.01.nc4",
        )
        ...
        for casename, case in cases.items():
            ...
            thisCrop1_gridded = utils.grid_one_variable(
                case_ds, thisVar, vegtype=vegtype_int
            ).squeeze(drop=True)
            ...
            if "rx_gslen_ds" in case:
                if len(clmExp_gslen_ds.dims) > 0:
                    raise RuntimeError(
                        "Need to amend figure code to allow for multiple prescribed GS lengths"
                    )
        ...
        for i, (casename, case) in enumerate(cases.items()):
            ...
            im = make_map(
                ax, map1_yx, fontsize, this_title=casename, lonlat_bin_width=bin_width, cmap=cmap
            )
            ims.append(im[0])
        ...
        if not diffExpected:
            ...
            im = make_map(
                ...
                clmExp_gslen_yx,
                fontsize,
                this_title="CLM expected",
                lonlat_bin_width=bin_width,
                cmap=cmap,
            )
            ims.append(im[0])
        ...
        for g in np.arange(Nggcmi_models):
            ...
            if verbose:
                print(
                    f"{ggcmi_models[g]} map ranges {np.nanmin(ggcmi_yx.values)} to"
                    f" {np.nanmax(ggcmi_yx.values)}"
                )
...
varList = {
    "PROD_ANN": {"suptitle": "Mean annual production", "units": "Mt"},
    "PROD_ANN_DIFF": {
        "suptitle": "Mean annual production",
        "units": "Mt",
        "maskcolorbar_near0": "percentile|5|cumulative",
    },
    "YIELD_ANN": {"suptitle": "Mean annual yield", "units": "t/ha"},
    "YIELD_ANN_DIFF": {"suptitle": "Mean annual yield", "units": "t/ha"},
    ...
    "GSLEN": {"suptitle": "Mean growing season length", "time_dim": "gs", "units": "days"},
    "GSLEN_DIFF": {
...
for this_var, var_info in varList.items():
    cases = maps_allCrops(
        ...
        var_info,
        outDir_figs,
        cropList_combined_clm_nototal,
        figsize,
        earthstats=earthstats,
        min_viable_hui=min_viable_hui,
        ny=ny,
        nx=nx,
        croptitle_side=croptitle_side,
        crop_subset=crop_subset,
    )
