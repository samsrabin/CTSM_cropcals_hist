...
def main(argv):
    ...
    if not args.only_make_figs:
        ...
        gddfn.log(
            logger,
            (
                f"Importing netCDF time steps {y1_import_str} through {yN_import_str} (years are +1"
                " because of CTSM output naming)"
            ),
        )

        pickle_file = os.path.join(
            args.output_dir, f"{args.first_season}-{args.last_season}.pickle"
        )
        h1_ds_file = os.path.join(
            args.output_dir, f"{args.first_season}-{args.last_season}.h1_ds.nc"
        )
    ...
    if not args.only_make_figs:
        ...
        def save_gdds(args, outfile, gdd_maps_ds, sdates_rx):
            ...
            comment = (
                ...
                f" {os.path.basename(args.sdates_file) and {os.path.basename(args.hdates_file)}}."
            )
            gdd_maps_ds.attrs = {
    ...
    if not args.only_make_figs:
        ...
        gdd_maps_ds = add_attrs_to_map_ds(
            gdd_maps_ds, plot_vegtypes_str, dummy_fill, outdir_figs, args
        )
        gddharv_maps_ds = add_attrs_to_map_ds(
            gddharv_maps_ds, plot_vegtypes_str, dummy_fill, outdir_figs, args
        )
    ...
    def make_figures(
        args, thisDir=None, gdd_maps_ds=None, gddharv_maps_ds=None, outdir_figs=None, linewidth=1.5
    ):
        if not gdd_maps_ds:
            if not thisDir:
                gddfn.error(
        ...
        for v, vegtype_str in enumerate(
            incl_vegtypes_str + ["Corn", "Cotton", "Rice", "Soybean", "Sugarcane", "Wheat"]
        ):
            print(f"{vegtype_str}...")
